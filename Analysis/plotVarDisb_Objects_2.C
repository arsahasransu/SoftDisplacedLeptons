...
public:
  ...
  // Multiplicities
  std::vector<TH1F*> hist_NLep;  
  std::vector<TH1F*> hist_NEl;  
  std::vector<TH1F*> hist_NMu;  
  std::vector<TH1F*> hist_NJet;  
  
  // Regular Lepton Kinematics
  std::vector<TH1F*> hist_PT_Lep;
  std::vector<TH1F*> hist_PT_El;
  std::vector<TH1F*> hist_PT_Mu;
  std::vector<TH1F*> hist_PT_Jet;
  std::vector<TH1F*> hist_leadPT_Lep;
  std::vector<TH1F*> hist_leadPT_El;
  std::vector<TH1F*> hist_leadPT_Mu;
  std::vector<TH1F*> hist_leadPT_Jet;
  std::vector<TH1F*> hist_subLeadPT_Lep;
  std::vector<TH1F*> hist_subLeadPT_El;
  std::vector<TH1F*> hist_subLeadPT_Mu;
  std::vector<TH1F*> hist_subLeadPT_Jet;
  std::vector<TH1F*> hist_diffPT_Lep;
  std::vector<TH1F*> hist_diffPT_Jet;
...
plotVarDisb_Objects::plotVarDisb_Objects(int fileTypeNum) {
  ...
  for(int fileTypeCtr=0; fileTypeCtr<fileTypeNum; fileTypeCtr++) {
    ...
    // Multiplicities
    hist_NLep.push_back(new TH1F("NLepton_"+histSuff[fileTypeCtr],"",11,-0.5,10.5));  
    hist_NEl.push_back(new TH1F("NElectron_"+histSuff[fileTypeCtr],"",11,-0.5,10.5));  
    hist_NMu.push_back(new TH1F("NMuon_"+histSuff[fileTypeCtr],"",11,-0.5,10.5));  
    hist_NJet.push_back(new TH1F("NJet_"+histSuff[fileTypeCtr],"",11,-0.5,10.5));  
    
    // Regular Lepton Kinematics
    hist_PT_Lep.push_back(new TH1F("PT_Lepton"+histSuff[fileTypeCtr],"",51,-1,101));
    ...
    hist_leadPT_Lep.push_back(new TH1F("leadPT_Lepton"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_leadPT_El.push_back(new TH1F("leadPT_El"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_leadPT_Mu.push_back(new TH1F("leadPT_Mu"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_leadPT_Jet.push_back(new TH1F("leadPT_Jet"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_subLeadPT_Lep.push_back(new TH1F("subLeadPT_Lepton"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_subLeadPT_El.push_back(new TH1F("subLeadPT_El"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_subLeadPT_Mu.push_back(new TH1F("subLeadPT_Mu"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_subLeadPT_Jet.push_back(new TH1F("subLeadPT_Jet"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_diffPT_Lep.push_back(new TH1F("diffPT_Lept"+histSuff[fileTypeCtr],"",51,-1,101));
    hist_diffPT_Jet.push_back(new TH1F("diffPT_Jet"+histSuff[fileTypeCtr],"",51,-1,101));
  }
}
...
void plotVarDisb_Objects::readTreeFillHist(TChain *tree, int fileTypeCtr) {
  ...
  for(int evtCtr=0; evtCtr<tree->GetEntries() /*&& evtCtr<10*/; evtCtr++) {
    ...
    hist_NLep[fileTypeCtr]->Fill(numGoodLep);
    hist_NEl[fileTypeCtr]->Fill(numGoodEl);
    hist_NMu[fileTypeCtr]->Fill(numGoodMu);      
    ...
    for(int objCtr=0; objCtr<lenObj; objCtr++) {
      ...
      hist_PT_Lep[fileTypeCtr]->Fill(PT->at(objCtr));
      if(TMath::Abs(PID->at(objCtr))==11) hist_PT_Mu[fileTypeCtr]->Fill(PT->at(objCtr));
      if(TMath::Abs(PID->at(objCtr))==13) hist_PT_El[fileTypeCtr]->Fill(PT->at(objCtr));
    }

    hist_leadPT_Lep[fileTypeCtr]->Fill(PT->at(firstPos));
    hist_subLeadPT_Lep[fileTypeCtr]->Fill(PT->at(secondPos));
    hist_diffPT_Lep[fileTypeCtr]->Fill(TMath::Abs(PT->at(firstPos)-PT->at(secondPos)));
    if(TMath::Abs(PID->at(firstPos))==11) {
      hist_leadPT_El[fileTypeCtr]->Fill(PT->at(firstPos));
      hist_subLeadPT_Mu[fileTypeCtr]->Fill(PT->at(secondPos));      
    }
    if(TMath::Abs(PID->at(firstPos))==13) {
      hist_leadPT_Mu[fileTypeCtr]->Fill(PT->at(firstPos));
      hist_subLeadPT_El[fileTypeCtr]->Fill(PT->at(secondPos));      
    }
    ...
    hist_NJet[fileTypeCtr]->Fill(numGoodJet);
    if(numGoodJet>=1) hist_leadPT_Jet[fileTypeCtr]->Fill(JetPT->at(jetFirstPos));
    if(numGoodJet>=2) {
      hist_subLeadPT_Jet[fileTypeCtr]->Fill(JetPT->at(jetSecondPos));
      hist_diffPT_Jet[fileTypeCtr]->Fill(JetPT->at(jetFirstPos)-JetPT->at(jetSecondPos));
      std::cout<<JetPT->at(jetFirstPos)<<"\t"<<JetPT->at(jetSecondPos)<<std::endl;
    }
  } // End of Event Loop
...
void execute() {
  ...
  pVDO->plotBeautifier(pVDO->hist_NLep, histLabel, "No.of Leptons", "Number of Events scaled to unity", "NLep");  
  pVDO->plotBeautifier(pVDO->hist_NEl, histLabel, "No.of Electrons", "Number of Events scaled to unity", "NEl");  
  pVDO->plotBeautifier(pVDO->hist_NMu, histLabel, "No.of Muons", "Number of Events scaled to unity", "NMu");  
  pVDO->plotBeautifier(pVDO->hist_NJet, histLabel, "No.of Jet", "Number of Events scaled to unity", "NJet");  
  
  // Regular Lepton Kinematics
  pVDO->plotBeautifier(pVDO->hist_PT_Lep, histLabel, "pT_{lep}", "Number of Events scaled to unity", "PT_Lep");
  ...
  pVDO->plotBeautifier(pVDO->hist_leadPT_Lep, histLabel, "pT^{lead}_{lep}", "Number of Events scaled to unity", "PT_LeadLep");
  pVDO->plotBeautifier(pVDO->hist_leadPT_El, histLabel, "pT^{lead}_{e}", "Number of Events scaled to unity", "PT_LeadEl");
  pVDO->plotBeautifier(pVDO->hist_leadPT_Mu, histLabel, "pT^{lead}_{#mu}", "Number of Events scaled to unity", "PT_LeadMu");
  pVDO->plotBeautifier(pVDO->hist_leadPT_Jet, histLabel, "pT^{lead}_{jet}", "Number of Events scaled to unity", "PT_LeadJet");
  pVDO->plotBeautifier(pVDO->hist_subLeadPT_Lep, histLabel, "pT^{sub-lead}_{lep}", "Number of Events scaled to unity", "PT_SubLeadLep");
  pVDO->plotBeautifier(pVDO->hist_subLeadPT_El, histLabel, "pT^{sub-lead}_{e}", "Number of Events scaled to unity", "PT_SubLeadEl");
  pVDO->plotBeautifier(pVDO->hist_subLeadPT_Mu, histLabel, "pT^{sub-lead}_{#mu}", "Number of Events scaled to unity", "PT_SubLeadMu");
  pVDO->plotBeautifier(pVDO->hist_subLeadPT_Jet, histLabel, "pT^{sub-lead}_{jet}", "Number of Events scaled to unity", "PT_SubLeadJet");
  pVDO->plotBeautifier(pVDO->hist_diffPT_Lep, histLabel, "#DeltapT_{lep}", "Number of Events scaled to unity", "DeltaPT_Lep");
  pVDO->plotBeautifier(pVDO->hist_diffPT_Jet, histLabel, "#DeltapT_{jet}", "Number of Events scaled to unity", "DeltaPT_Jet");
