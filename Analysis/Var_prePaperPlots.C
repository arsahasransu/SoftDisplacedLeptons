...
plotVarDisb_Objects::plotVarDisb_Objects(int fileTypeNum) {
  ...
  // Define Histograms
  for(signed int fileTypeCtr=0; fileTypeCtr<fileTypeNum; fileTypeCtr++) {
...
void plotVarDisb_Objects::readTreeFillHist(TChain *tree, int fileTypeCtr, bool isSignal) {
  ...
  // Open Event Loop
  for(unsigned int evtCtr=0; evtCtr<tree->GetEntries(); evtCtr++) {
    ...
    for(unsigned int objCtr=0; objCtr<lenObj; objCtr++) {
    ...
    hist_dRll[fileTypeCtr]->Fill(TMath::Abs(lep1.DeltaR(lep2)), d0wt);
    ...
    int jetFirstPos = -1;
    int jetSecondPos = -1;
    for(signed int jetCtr=0; jetCtr<numJet; jetCtr++) {
    ...
    hist_MtLeadLepMET[fileTypeCtr]->Fill(TMath::Sqrt(2*MET*PT->at(firstPos)*(1-TMath::Cos(lep1.DeltaPhi(METvec)))), d0wt);
    if(htjet!=0) hist_HTJet[fileTypeCtr]->Fill(htjet, d0wt);
    hist_dPhiLepMETSelObj[fileTypeCtr]->Fill(TMath::Abs(lep1.DeltaPhi(objSum)), d0wt);
    hist_YDelpObj[fileTypeCtr]->Fill(MET/TMath::Sqrt(HT), d0wt);
    hist_YUserObj[fileTypeCtr]->Fill(TMath::Abs(objSum.Pt())/TMath::Sqrt(htlep), d0wt);

    double mt = TMath::Sqrt(htlep*htlep-lepSum.Pt()*lepSum.Pt());
    hist_alphaT[fileTypeCtr]->Fill(PT->at(secondPos)/mt, d0wt);
    hist_Sphericity[fileTypeCtr]->Fill(sphericity(lepvec), d0wt);
    hist_Spherocity[fileTypeCtr]->Fill(transversespherocity(lepvec), d0wt);
...
void plotVarDisb_Objects::plotBeautifier(std::vector<TH1F*> hist, std::vector<TString> label,
  ...
  pMain->cd();
  for(unsigned int histCtr=0; histCtr<hist.size(); histCtr++) {
  ...
  for(unsigned int histCtr=0; histCtr<1/*hist.size()/2.0*/; histCtr++) {
  ...
  */
  ...
  for(unsigned int histCtr=1/*(int)(hist.size()/2.0)*/; histCtr<3/*hist.size()*/; histCtr++) {
