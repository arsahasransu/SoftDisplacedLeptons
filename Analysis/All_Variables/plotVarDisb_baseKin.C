...
plotVarDisb_Objects::plotVarDisb_Objects(int fileTypeNum) {
  ...
  for(int fileTypeCtr=0; fileTypeCtr<fileTypeNum; fileTypeCtr++) {
    ...
    hist_D0_El.push_back(new TH1F("D0_El"+histSuff[fileTypeCtr],"",51,0,0.01));
    hist_D0_Mu.push_back(new TH1F("D0_Mu"+histSuff[fileTypeCtr],"",51,0,0.01));
    hist_Log10D0_El.push_back(new TH1F("Log10D0_El"+histSuff[fileTypeCtr],"",51,-6,1));
    hist_Log10D0_Mu.push_back(new TH1F("Log10D0_Mu"+histSuff[fileTypeCtr],"",51,-6,1));
    hist_Iso_El.push_back(new TH1F("Iso_El"+histSuff[fileTypeCtr],"",51,-1,1));
    hist_Iso_Mu.push_back(new TH1F("Iso_Mu"+histSuff[fileTypeCtr],"",51,-1,1));
...
void plotVarDisb_Objects::plotBeautifier(std::vector<TH1F*> hist, std::vector<TString> label, TString XaxisTitle, TString YaxisTitle, TString saveName) {
  ...
  c1->SaveAs("./baseKin/"+saveName+".pdf");
}
...
void execute() {
  ...
  histLabel.push_back("Displaced Lepton Signal BP(304,324)");
  TChain *t0 = new TChain("SelectedObjects");
  t0->Add("/home/arsahasransu/Documents/SoftDisplacedLeptons/Data/DislacedLepton/Objects_sorted_DisplacedLepton_*.root");
  cout<<"Initialized "<<histLabel[0]<<" chain"<<endl;
  
  histLabel.push_back("pp > bb~ HF Background");
  TChain *t1 = new TChain("SelectedObjects");
  t1->Add("/home/arsahasransu/Documents/SoftDisplacedLeptons/Data/ppTobb_Cuts2/Objects_sorted_ppTobb_Cuts2_*.root");
  cout<<"Initialized "<<histLabel[1]<<" chain"<<endl;

  histLabel.push_back("Displaced Lepton Signal BP(100,200)");
  TChain *t2 = new TChain("SelectedObjects");
  t2->Add("/home/arsahasransu/Documents/SoftDisplacedLeptons/Data/DislacedLepton_BP_100_200/Objects_sorted_DisplacedLepton_BP_100_200_*.root");
  cout<<"Initialized "<<histLabel[2]<<" chain"<<endl;

  std::cout<<"No.of  "<<histLabel[0]<<" Entries: "<<t0->GetEntries()<<std::endl;
  std::cout<<"No.of  "<<histLabel[1]<<" Entries: "<<t1->GetEntries()<<std::endl;
  std::cout<<"No.of  "<<histLabel[2]<<" Entries: "<<t2->GetEntries()<<std::endl;
  ...
  pVDO->readTreeFillHist(t2, 2);
  cout<<"Filled corresponding "<<histLabel[2]<<" histograms"<<endl;
