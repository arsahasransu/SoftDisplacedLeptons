...
print("number of branches: ", len(listofbranches))
corHist = rt.TH2F("VariableCorelations","",12,-0.5,11.5,12,-0.5,11.5)
corHist.SetStats(0)
corHist.SetCanExtend(rt.TH2.kAllAxes)
for p1 in list(perm):
    ...
    workhist = rt.TH2D("workhist"+workname1+workname2,"",40,min1,max1,40,min2,max2)
    ...
    sigcor = round(workhist.GetCorrelationFactor(),2)
    bkgcor= round(workhistbg.GetCorrelationFactor(),2)
    ...
    print("Finished", workname1)
    corHist.Fill(workname1,workname2,abs(sigcor))
    corHist.Fill(workname2,workname1,abs(bkgcor))

canv = rt.TCanvas()
corHist.Draw("COLZ TEXT")
canv.SaveAs("corhist.pdf")
input("Press Enter to continue...")
